...
const MEMBERS = gql`
query{
  members(where: {committee_contains_some: FashionDesigners}, orderBy: displayOrder_ASC) {
    id
    memberName
    memberPosition
    slug
...
export default function FashionDesigners() {
  ...
  const [ fashionDesigners, setfashionDesigners ] = useState();


  useEffect(() => {
    if(loading === false && data){
        setfashionDesigners(data);
    }
}, [loading, data])
...
if (error) return <div className="error-bio"><h1>{error.message}</h1></div>;
  if (data) {
      return (
        <SEO
        pageMeta={{
        title: "Fashion Designers | Agency",
